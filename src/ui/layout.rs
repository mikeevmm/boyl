...
pub fn distribute(max_width: u16, elements: &[VisualBox]) -> Vec<Position> {
    let splits = {
        type Badness = u64;
        let mut break_memo = HashMap::<usize, usize>::new();
        let mut badness_memo = HashMap::<usize, Badness>::new();
        ...
        while !start_stack.is_empty() {
            ...
            let suffix_badness = {
                if newline_before == elements.len() {
                    ...
                    match badness_memo.get(&newline_before) {
            ...
            if newline_before == elements.len() {
                ...
                badness_memo.insert(start, best_badness);
                break_memo.insert(start, best_break);
                continue;
            }
        ...
        while head < elements.len() {
            let next_break = *break_memo.get(&head).unwrap();
